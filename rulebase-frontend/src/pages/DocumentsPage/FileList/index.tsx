...
function getFileIcon(mimeType: string): string {
  ...
  if (mimeType.includes('hwp') || mimeType.includes('haansoft')) return '📃';
  return '📁';
}
...
export function FileList() {
  ...
  // Fetch unresolved Q&A counts per file in a single batch (admin only)
  const fileIds = files?.map(f => f.id) ?? [];
  const { data: unresolvedCounts } = useQuery({
    queryKey: ['questions', 'unresolved-counts', fileIds.join(',')],
    queryFn: async () => {
      if (fileIds.length === 0) return [];
      const res = await questionsApi.getUnresolvedCounts(fileIds);
      return res.data.data as { file_id: number; unresolved_count: number }[];
    },
    enabled: isAdmin && fileIds.length > 0,
  });

  const unresolvedCountMap = new Map<number, number>();
  unresolvedCounts?.forEach(item => {
    if (item.unresolved_count > 0) unresolvedCountMap.set(item.file_id, item.unresolved_count);
  });
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1">
                <p className="text-sm font-medium text-gray-800 truncate" title={file.original_name}>{file.original_name}</p>
                {isAdmin && unresolvedCountMap.has(file.id) && (
                  <span className="flex-shrink-0 text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-red-500 text-white leading-none" title={`미해결 Q&A ${unresolvedCountMap.get(file.id)}건`}>
                    {unresolvedCountMap.get(file.id)}
                  </span>
                )}
              </div>
